  ...
  const sections = [
    {
      ...
      stats: [
        ...
        { label: 'High BPM music boosts mood and reduces stress', highlight: false },
      ],
      color: 'purple',
    },
    {
    ...
    <div className="space-y-8">
      {sections.map((section, index) => (
        <Card 
          ...
          hover={false}
        >
          {/* Animation wrapper since Card cannot take style */}
          <div style={{ animationDelay: `${index * 0.1}s` }}>
            <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  {section.stats.map((stat, statIndex) => (
                    <div
                      ...
                      className={`flex items-center justify-center md:justify-start space-x-2 ${
                        stat.highlight
                          ? 'text-uw-gold-light font-semibold'
                          : 'text-uw-gold/80'
                      }`}
                    >
                      <span
                        className={
                          stat.highlight ? 'text-uw-gold' : 'text-uw-gold/50'
                        }
                      >
                        {stat.highlight ? '★' : '•'}
                      </span>
              ...
              <div
                className={`hidden lg:flex items-center justify-center w-24 h-24 rounded-full ${
                  section.color === 'purple'
                    ? 'bg-uw-purple/20 border-2 border-uw-purple'
                    : 'bg-uw-gold/20 border-2 border-uw-gold'
                }`}
              >
                <span
